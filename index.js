...
(function () {
  ...
  function renderTrust() {
    ...
    el.innerHTML = D.trustBadges.map(b => `
      <div class="trust__item reveal-element">
        <span class="trust__value">${b.value}</span>
        <span class="trust__label">${b.text}</span>
      </div>
    `).join("");
  }
  ...
  function renderWhy() {
    ...
    el.innerHTML = D.why.map(w => `
      <div class="why__card reveal-element">
        ...
        <h3 class="why__card-title">${w.title}</h3>
        <p class="why__card-text">${w.text}</p>
      </div>
    `).join("");
  }
  ...
  function renderServices() {
    ...
    el.innerHTML = D.services.map(s => `
      <article class="service-card${s.featured ? " service-card--featured" : ""} reveal-element">
        <img class="service-card__image" src="${s.image}" alt="${s.title}" loading="lazy">
        <div class="service-card__overlay"></div>
        <div class="service-card__content">
          <h3 class="service-card__title">${s.title}</h3>
          <p class="service-card__desc">${s.desc}</p>
          <span class="service-card__price">от ${formatPrice(s.priceFrom)}</span>
        </div>
      </article>
    `).join("");
  }
    ...
    el.innerHTML = D.cases.map((c, i) => `
      <div class="case-card reveal-element" data-case="${i}">
        <div class="case-card__slider">
          <img class="case-card__img case-card__img--after" src="${c.afterImage}" alt="После: ${c.title}" loading="lazy">
          <img class="case-card__img case-card__img--before" src="${c.beforeImage}" alt="До: ${c.title}" loading="lazy">
        ...
        <div class="case-card__info">
          ...
          <p class="case-card__text">${c.caption} • Срок: ${c.duration}</p>
        </div>
      </div>
    `).join("");
  }
  ...
  function renderDoctors() {
    ...
    el.innerHTML = D.doctors.map(d => `
      <div class="doctor-card reveal-element">
        <img class="doctor-card__photo" src="${d.photo}" alt="${d.name}" loading="lazy">
        <div class="doctor-card__info">
          <h3 class="doctor-card__name">${d.name}</h3>
          <p class="doctor-card__role">${d.role}</p>
          <p class="doctor-card__exp">${d.experience}</p>
        </div>
      </div>
    `).join("");
  }
    ...
    el.innerHTML = D.testimonials.map(t => `
      <div class="swiper-slide">
        <div class="testimonial-card">
          <div class="testimonial-card__stars">${starIcons(t.rating || 5)}</div>
          <p class="testimonial-card__text">"${t.text}"</p>
          <div class="testimonial-card__author">
            <div>
              <p class="testimonial-card__name">${t.name}</p>
              <p class="testimonial-card__service">${t.service || ""} · ${t.source || ""}</p>
            </div>
          </div>
        </div>
      </div>
  ...
  function renderPrices() {
    ...
    el.innerHTML = D.prices.map(p => `
      <div class="price-card${p.accent ? " price-card--accent" : ""} reveal-element">
        ...
        <p class="price-card__amount">${p.priceFrom === 0 ? "Бесплатно" : "от " + formatPrice(p.priceFrom)}</p>
  ...
  function renderQuiz() {
    ...
    const stepsHTML = steps.map((s, i) => `
      <div class="quiz__step${i === 0 ? " is-active" : ""}" data-quiz-step="${i}">
        <p class="quiz__question">${s.question}</p>
        <div class="quiz__options">
          ${s.options.map(o => `<button type="button" class="quiz__option">${o}</button>`).join("")}
        </div>
        <div class="quiz__nav">
    ...
    const finalHTML = `
      <div class="quiz__step" data-quiz-step="final">
        <p class="quiz__question">${D.quiz.finalBonusText}</p>
  ...
  function renderContacts() {
    ...
    const secondPhone = c.phoneSecondary
      ? `<br><a href="tel:${c.phoneSecondaryRaw}">${c.phoneSecondary}</a>`
      : "";
    ...
    el.innerHTML = `
      <div class="contact-item">
        ...
        <div>
          ...
          <p class="contact-item__value"><a href="tel:${c.phoneRaw}">${c.phone}</a>${secondPhone}</p>
        </div>
      </div>
      <div class="contact-item">
        ...
        <div>
          ...
          <p class="contact-item__value"><a href="mailto:${c.email}">${c.email}</a></p>
        </div>
      </div>
      <div class="contact-item">
        ...
        <div>
          ...
          <p class="contact-item__value">${c.address}${c.addressSecondary ? `<br>${c.addressSecondary}` : ""}</p>
        </div>
      </div>
      <div class="contact-item">
        ...
        <div>
          ...
          <p class="contact-item__value">${c.hours}</p>
        </div>
      </div>
    `;
  ...
  function renderHeroImage() {
    ...
    el.innerHTML = `<img src="${D.heroImage}" alt="${D.brand || "Lumina Dental"} — красивая улыбка" width="600" height="450">`;
  }
    ...
    container.addEventListener("submit", (e) => {
      ...
      if (!nameVal || !phoneVal) {
        form.querySelectorAll(".input").forEach(inp => {
          if (!inp.value.trim()) inp.classList.add("is-error");
        });
        return;
      }
      ...
      form.innerHTML = `
                <div class="form-status form-status--success" style="text-align:center;">
                    <p style="font-size:1.125rem;font-weight:600;margin-bottom:8px;">✓ Сообщение готово!</p>
                    <p>Мы подготовили сообщение в WhatsApp — нажмите «Отправить» в открывшемся окне.</p>
                    <a href="${waUrl}" target="_blank" rel="noopener" class="btn btn--primary" style="margin-top:16px;">Открыть WhatsApp ещё раз</a>
                </div>`;
    });
  }
  ...
  function initHeroForm() {
    ...
    form.addEventListener("submit", (e) => {
      ...
      const submitBtn = form.querySelector("#hero-form-submit");
      if (submitBtn) {
      ...
      setTimeout(() => {
        ...
        status.textContent = `Спасибо, ${name.value.trim()}! Мы подготовили сообщение в WhatsApp — нажмите «Отправить».`;
